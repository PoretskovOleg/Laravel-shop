            ...
            <section class="flex flex-col lg:flex-row gap-10 xl:gap-14 2xl:gap-20 mt-12">
                ...
                <div class="basis-full lg:basis-2/5 xl:basis-2/4">
                    <div class="overflow-hidden h-auto max-h-[620px] lg:h-[480px] xl:h-[620px] rounded-3xl">
                        <img src="{{ $product->makeThumbnail('345x320') }}"
                    ...
                    <div class="grow flex flex-col lg:py-8">
                        ...
                        @if(!empty($product->json_properties))
                            <ul class="sm:max-w-[360px] space-y-2 mt-8">
                                @foreach($product->json_properties as $propertyTitle => $propertyValue)
                                    <li class="flex justify-between text-body">
                                        <strong class="text-white">{{ $propertyTitle }}:</strong>
                                        {{ $propertyValue }}
                                    </li>
                                @endforeach
                            </ul>
                                ...
                                @foreach($options as $optionTitle => $optionValues)
                                    <div class="flex flex-col gap-2">
                                        <label for="filter-item-1"
                                               class="cursor-pointer text-body text-xxs font-medium">{{ $optionTitle }}</label>
