            ...
            <div class="basis-auto xl:basis-3/4">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ filter_url($category, ['view' => 'grid']) }}"
                               class="{{ is_catalog_view('grid') ? 'pointer-events-none text-pink' : 'text-white hover:text-pink' }} inline-flex items-center justify-center w-10 h-10 rounded-md bg-card"
                            >
                                <svg class="w-5 h-5"
                            ...
                            <a href="{{ filter_url($category, ['view' => 'list']) }}"
                               class="{{ is_catalog_view('list') ? 'pointer-events-none text-pink' : 'text-white hover:text-pink' }} inline-flex items-center justify-center w-10 h-10 rounded-md bg-card"
                            >
                                <svg class="w-5 h-5"
                                     xmlns="http://www.w3.org/2000/svg"
                                     fill="currentColor"
                    ...
                    <div x-data="{sort: '{{ filter_url($category, ['sort' => request('sort')]) }}'}"
                        ...
                        <form x-ref="sortForm" action="{{ route('catalog', $category) }}" method="GET">
                            <select name="sort"
                                    x-model="sort"
                            ...
                            >
                                <option value="{{ filter_url($category, ['sort' => '']) }}" class="test-dark">
                                    умолчанию
                                </option>
                                <option value="{{ filter_url($category, ['sort' => 'price']) }}" class="test-dark">
                                    от дешевых к дорогим
                                </option>
                                <option value="{{ filter_url($category, ['sort' => '-price']) }}" class="test-dark">
                                    от дорогих к дешевым
                                </option>
                                <option value="{{ filter_url($category, ['sort' => 'title']) }}" class="test-dark">
                                    наименованию
                                </option>
                            </select>
                        </form>
                ...
                <!-- Products list -->
                <div class="products grid grid-cols-1 @if(is_catalog_view('list')) gap-y-8 @else sm:grid-cols-2 xl:grid-cols-3 gap-x-6 2xl:gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12 @endif">
